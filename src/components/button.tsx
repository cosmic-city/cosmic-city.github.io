...
function Button({
  ...
  className?: string;
  onClick?: () => void;
  look?: "default" | "brand" | "space" | "subscribe" | "discord";
}) {
  const before =
  ...
  return (
    <button
      ...
      className={`${before} ${look == "default" || !look ? defaultLook : look == "brand" ? brandLook : look == "subscribe" ? "py-2 px-4 rounded-md bg-red-600 drop-shadow-md text-white uppercase font-semibold" : look == "discord" ? "py-2 px-4 rounded-md bg-violet-600 drop-shadow-md text-white uppercase font-semibold" : look == "space" ? "py-2 px-4 rounded-3xl bg-[#fff] font-semibold hover:bg-[#ddd] text-black" : defaultLook} ${className && className}`}
    >
      {look == "brand" ? (
